...
def create_project(
    db: Session, name: str, description: str | None = None, owner_id: int | None = None
):
...
def list_projects(db: Session, user_id: int):
    ...
    rows = (
        db.query(
            models.Project.id,
            models.Project.name,
            func.count(models.Site.id).label("site_count"),
        )
        .outerjoin(models.Site, models.Site.project_id == models.Project.id)
...
def create_site(
    db: Session, project_id: int, name: str, description: str, geojson: dict
):
    geom = from_shape(shape(geojson["geometry"]), srid=4326)
    site = models.Site(
        project_id=project_id, name=name, description=description, geom=geom
    )
    db.add(site)
    db.commit()
    db.refresh(site)
...
def list_sites_geojson(db: Session, project_id: int):
    q = (
        db.query(
            models.Site.id,
            models.Site.name,
            models.Site.description,
            func.ST_AsGeoJSON(models.Site.geom).label("geojson"),
        )
...
def get_site(db: Session, site_id: int):
    r = (
        db.query(models.Site, func.ST_AsGeoJSON(models.Site.geom).label("geojson"))
...
def site_area_ha(db: Session, site_id: int) -> float:
    q = (
        db.query(func.ST_Area(func.Geography(models.Site.geom)))
        .filter(models.Site.id == site_id)
        .scalar()
    )
    if q is None:
...
def _create_project(
    ...
    p = create_project(
        db, name=payload.name, description=payload.description, owner_id=user.id
    )
    return p
...
def _get_site(
    site_id: int, db: Session = Depends(get_db), authorization: str = Header(None)
):
    _ = get_current_user_from_header(authorization, db)
